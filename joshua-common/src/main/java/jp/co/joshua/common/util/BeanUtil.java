...
public class BeanUtil {
            ...
            for (Field targetField : BeanUtil.getFieldList(destClass)) {
                ...
                for (Field sourceField : BeanUtil.getFieldList(srcClass)) {
                    if (isCopyTarget(sourceField, targetField)) {
                        // getter呼び出し
                        Method getter = getAccessor(sourceField.getName(),
                                srcClass, AccessorType.GETTER);
                        // setter呼び出し
                        Method setter = getAccessor(sourceField.getName(),
                                destClass, AccessorType.SETTER);
    ...
    public static Class<?> getParameterType(Class<?> clazz, int position) {
        ParameterizedType paramType = (ParameterizedType) clazz
